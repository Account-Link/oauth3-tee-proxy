...
class TwitterCookieAuthorizationPlugin(TwitterBaseAuthorizationPlugin):
    ...
    async def get_user_identifier(self, credentials: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if not user_id:
                raise ValueError("Could not extract user ID from Twitter cookie response")
            return str(user_id)  # Convert to string to ensure consistent type
        except Exception as e:
    ...
    def credentials_from_string(self, credentials_str: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Error parsing Twitter cookie credentials: {e}")
            raise ValueError("Invalid Twitter cookie credentials format") from e